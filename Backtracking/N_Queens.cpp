...
/*
    ...
    Before placing a queen, check:
        ...
        If the upper left diagonal is clear.
        If the upper right diagonal is clear.
...
bool check(int &n, vector<string> &Board, int &i, int &j){
    ...
    // check kro ki kahiUpper Left Diagonal me queen to nhi rkhi?
    while(row > -1 && col > -1){

        // Agar uss position me queen hai to nhi rkh skte
        if(Board[row][col] == 'Q')
            return false;
    ...
    // reinitialize the row and col
    row = i, col = j;

    // Ab Upper Right Diagonal me dekho
    while(row > -1 && col < n){
    ...
    // Agar diagonals me koi bhi queen present nhi hai means hum uss position me queen rkh skte hai
    return true;
...
void find(int row, int &n, vector<vector<string>> &ans, vector<string> &Board, vector<bool> &column){
    ...
    for(int i = 0; i < n; i++){
        ...
        if(column[i] == false && check(n, Board, row, i)){
